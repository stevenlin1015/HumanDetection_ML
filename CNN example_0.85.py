...
trainLabel_oneshot = np_utils.to_categorical(trainL, 2)
testData_normalize = testD.reshape(testData.shape[0], testData.shape[1], testData.shape[2], 3).astype('uint8')
testLabel_oneshot = np_utils.to_categorical(testL, 2)
...
model = Sequential()
model.add(Conv2D(filters = 16, kernel_size = (3, 3), input_shape = (trainData_normalize.shape[1], trainData_normalize.shape[2], 3), data_format = "channels_last"))
