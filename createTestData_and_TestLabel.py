...
test_data = np.zeros((6000, 199, 300, 1), dtype = np.float32)
test_label = np.zeros((6000, 1), dtype = np.uint8)
...
def CreateTrainData_and_TrainLabel(testDataFolder):
    ...
    for root, dirs, files in os.walk(testDataFolder):
        for fileName in files:
            ...
            processingImage = cv.imread(testDataFolder + '/' + fileName, 0)            
            ...
            if processingImage.shape[0] < processingImage.shape[1]:
                for height in range(processingImage.shape[0]):
                    for width in range(processingImage.shape[1]):
                        test_data[imageIndexCount, height, width, 0] = processingImage[height, width]
                        if imageIndexCount < 3000:
                ...
                processingImage = adjustPortraitImage.ExportCorrectPortraitImageSize(testDataFolder + '/' + fileName)
...
CreateTrainData_and_TrainLabel("/Volumes/KINGSTON/ML_TrainingSet/Catagory/testData")